...
int main(void)
{
  ...
  /* USER CODE BEGIN Init */
	BrushedDriverConfig bConfig = { .phase_a_port = GPIOC, .phase_a_pin =
	GPIO_PIN_6, .phase_b_port = GPIOC, .phase_b_pin = GPIO_PIN_7,
			.enFault_a_port =
			GPIOC, .enFault_a_pin =
			GPIO_PIN_13, .enFault_b_port = GPIOC, .enFault_b_pin =
			GPIO_PIN_14, .pwm_a_inst = &htim2, .pwm_a_channel =
...
//    BrushedComms_Process();
		...
		if (AMT222C_UpdatePosition(&mEncA) == AMT222C_OK) {
			pos = AMT222C_GetPosition(&mEncA);
			// Use pos: e.g. log, control motor, send over UART, etc.
		}
		STSPIN948_SetOutputs(&bDriver);
		STSPIN948_ReadInputs(&bDriver);
		bFeedback.motor_position[0] = (uint16_t) fabs(pos);
...
static void MX_GPIO_Init(void)
{
  ...
  HAL_GPIO_WritePin(GPIOC, GPIO_PIN_15|GPIO_PIN_5|GPIO_PIN_6|GPIO_PIN_7

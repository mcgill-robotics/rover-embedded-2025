...
void BrushedComms_ReceiveByte(uint8_t byte) {
	...
	if (expected_length > 0 && rx_index >= expected_length) {
		// Validate CRC
		uint8_t calc_crc = BrushedComms_CalculateCRC(&rx_buffer[1],
				expected_length - 2);
...
void BrushedComms_SendFeedback(const FeedbackData *feedback) {
	...
	memcpy(&packet[index], feedback->motor_position, 8);
	index += 8;
	memcpy(&packet[index], feedback->motor_current, 8);
	index += 8;
...
void BrushedComms_SendEcho(const uint8_t *data, uint16_t len) {
	...
	packet[index++] = START_BYTE;
	packet[index++] = CMD_ECHO;
	packet[index++] = len;

	memcpy(&packet[index], data, len);
	index += len;

	uint8_t crc = BrushedComms_CalculateCRC(&packet[1], len + 2);
	packet[index++] = crc;

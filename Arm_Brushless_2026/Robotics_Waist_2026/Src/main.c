...
/* motor information */
float gearRatio = 55.0f; // 1:100 Interpreted as 100 rotations of input shaft = 1 rotation of output
...
//	velCtrlSetPositionLimits(velCtrl, degreesToRad(-90.0f), degreesToRad(90.0f));
	...
	/* Start the 1 kHz processing timer */
...
void HAL_FDCAN_RxFifo0Callback(FDCAN_HandleTypeDef *hfdcan, uint32_t RxFifo0ITs) {
	if ((RxFifo0ITs & FDCAN_IT_RX_FIFO0_NEW_MESSAGE) != 0) {
		if (HAL_FDCAN_GetRxMessage(hfdcan, FDCAN_RX_FIFO0, &can_rx_header,
				can_rx_data) == HAL_OK) {
			can_rx_flag = 1;
		}
	}

...
//	uart_debug_print("Parsing the ID...\r\n");
	...
	uint16_t msg_ID = rxHeader->StdId & 0x07ff ; // We only care about the first 11 bits here
...
void ControlSingleMotor(float newSpeed){
	...
	// gang this is tony. James's code with the weird motor states is below. The steering motors are not as complicated. We
	//are handling that with our own logic.

	float newPosition = newSpeed; // for clarity, since the steering motors are controlled by position, not speed.

	runSingleMotorV2(newPosition);
...
void runSingleMotorV2(float newSpeed){
	...
	float positionCmd = speedCmd;
	setPIDGoalA(positionCmd);
...
void sendCANResponse(ParsedCANID *CANMessageID, float information){
    ...
    txID |= (1 & 0x01) << SENDER_DEVICE_SHIFT; // This value is now always 1 since the esc is sending data.
...
//    txHeader.MessageMarker       = 0;
    ...
    // Send the response
    if (HAL_CAN_AddT(&hcan2, &txHeader, txData, NULL) != HAL_OK) {
        Error_Handler();
    }
}

...
#define WAIST_GEAR_RATIO 55 //THIS NUMBER IS NOT RIGHT LOL
#define SHOULDER_GEAR_RATIO 120
#define ELBOW_GEAR_RATIO 100


#define WAIST_LOWER_LIMIT  0
#define WAIST_UPPER_LIMIT 60
#define SHOULDER_LOWER_LIMIT 0
#define SHOULDER_UPPER_LIMIT 45
#define ELBOW_LOWER_LIMIT 0
#define ELBOW_UPPER_LIMIT 45
...
const float WAIST_ANGULAR_VELOCITY = 10.0; // Value is in degrees per second
const float SHOULDER_ANGULAR_VELOCITY = 5.0; // Value is in degrees per second
const float ELBOW_ANGULAR_VELOCITY = 5.0; // Value is in degrees per second
...
const float ELBOW_CALIBRATION_ANGULAR_VELOCITY = 2.0; // Value is in degrees per second
const float CALIBRATION_INCH = 5.0; // This value does not really matter directly, as it is converted into rad/s desired
...
//				////////////////////////////////////////////////////////////////////////////////////////////////////////
////				if (!isCalibrating){ // You can only call a speed if it is in a calibration sequence for safety
////					return;
////				}
...
bool CalibrateSingleMotor(void){
	...
	return true; // COMMENT THIS OUT TOO FOR PROPER CALIBTATION



	// UNCOMMENT BELOW AFTER INITIAL TESTING IS OVER

	//First see if the motor is already at the limit
	if(switch1_opened){


		// If that is the case, then slowly inch away from the limit switch until it is off of the switch
		while(switch1_opened){
			HAL_Delay(5);
			runSingleMotorPositionIncrement(CALIBRATION_INCH); // In theory this number can be anything really --> Modify sign if moving the wrong way
		}
		MC_StopMotor1(); //Motor at the correct spot to assign 0 angle pos
	...
	// In this case, the motor is not yet at the limit
	else{
		...
		runSingleMotorPositionIncrement(getCurrentPosition()); // Hold current position
		HAL_Delay(5);
...
void runSingleMotorPosition(float posSetpointDegrees){
	...
	float inputShaftDegreeSetpoint = OutputShaftDegreetoInput(posSetpointDegrees);
	float inputShaftPositionInRad = degreesToRad(inputShaftDegreeSetpoint);
	// After startup

//	float radSetpoint = degreesToRad(rawPosDegrees);
...
void runSingleMotorPositionIncrement(float degreeIncrement){
	...
	float inputShaftDegreeSetpoint = OutputShaftDegreetoInput(targetPositionDegrees);
	float inputShaftPositionInRad = degreesToRad(inputShaftDegreeSetpoint);
...
void lowerLimitSwitchHit(){
	...
	// Now slowly inch away from the switch as done previously
	while(switch1_opened){
		HAL_Delay(5);
		runSingleMotorPositionIncrement(CALIBRATION_INCH); // In theory this number can be anything really --> Modify sign if moving the wrong way
	}
...
void upperLimitSwitchHit(){
	...
	//Firstly Hold the current position
	MC_ProgramPositionCommandMotor1(getCurrentPosition(), 0); //Hold current position after Start
	HAL_Delay(250);
	// Now Back up a decent amount from the limit
	runSingleMotorPositionIncrement(-UPPER_LIMIT_SWITCH_HIT_BACKUP); // In theory this number can be anything really --> Modify sign if moving the wrong way
	HAL_Delay(computeTrajectoryTime(getCurrentPosition(), getCurrentPosition()- UPPER_LIMIT_SWITCH_HIT_BACKUP ));
	return;
}
...
float getCurrentPosition() {
	...
	float positionDegrees = dpp_to_mech_deg(DPPPosition);
	float positionAdjusted = inputShaftDegreetoOutput(positionDegrees);

...
#include "mc_config_common.h" // This is for the encoder obejct
#include "stm32g4xx_hal_adc.h" //For temperature
...
#define CALIBRATION_OFFSET_SPEED -100 //Speed motor uses to push itself back from a limit when calibration

#define WAIST_GEAR_RATIO 55 // 55 THIS NUMBER IS NOT RIGHT LOL
#define SHOULDER_GEAR_RATIO 120
#define ELBOW_GEAR_RATIO 100


#define WAIST_LOWER_LIMIT  -45
#define WAIST_UPPER_LIMIT 45
#define SHOULDER_LOWER_LIMIT 0
#define SHOULDER_UPPER_LIMIT 80
...
const float WAIST_ANGULAR_VELOCITY = 30.0; // Value is in degrees per second
const float SHOULDER_ANGULAR_VELOCITY = 5.0; // Value is in degrees per second
const float ELBOW_ANGULAR_VELOCITY = 25.0; // Value is in degrees per second

const float WAIST_CALIBRATION_ANGULAR_VELOCITY = 5.0; // Value is in degrees per second
const float SHOULDER_CALIBRATION_ANGULAR_VELOCITY = 2.0; // Value is in degrees per second
...
const float CALIBRATION_DEGREE_REDUCTION_SPEED = 0.2; // THIS VALUE IS FOR ELBOW --> MUST CHANGE FOR OTHERS //TODO  IS MAKE THIS AUTOMATIC
const float STARTING_POS = 35;
...
float afterCalibrationOffset = 0; //Number which is used for calibration offsetting
...
void Process_Single_ESC_Command (ParsedCANID *CANMessageID, uint8_t *rxData){
	...
	if (CANMessageID->commandType == ACTION_RUN){
		...
		case (RUN_POSITION_INCREMENT):
				...
				// This function exclusively does not need to be calibrated to move on
				////////////////////////////////////////////////////////////////////////////////////////////////////////
				...
				if (isCalibrated){
					...
					if (motorState == IDLE){
					    // Now restart safely without snap-back, internal reference accurately zeroed
					    if (!MC_StartMotor1()) {
				...
				////////////////////////////////////////////////////////////////////////////////////////////////////////
				ControlSingleMotorPositionIncrement(information);
				}
		...
		case (RUN_POSITION):
				...
				uart_debug_print("In case RUN_POSIITON\r\n");
				////////////////////////////////////////////////////////////////////////////////////////////////////////
				ControlSingleMotorPosition(information);
		...
		case (RUN_FOLLOW_POSITION):
				...
				uart_debug_print("In case RUN_POSIITON\r\n");
				////////////////////////////////////////////////////////////////////////////////////////////////////////
				ControlSingleMotorPositionFollower(information);
		...
		case(READ_CALIBRATION):
				...
				sendCANResponse(CANMessageID, motorCalibrated);
				break;
		...
		case(GET_TEMPERATURE):
				...
				sendCANResponse(CANMessageID, temp);
				break;
		case(GET_PING):
				float feedback = 69;// ;)
				...
				break;
		case(READ_POSITION):
				float positionAdjusted = getCurrentPosition();
...
void Process_Multiple_ESC_Command (ParsedCANID *CANMessageID, uint8_t *rxData){
	...
	switch(CANMessageID->runSpec){
			...
			case (RUN_POSITION_INCREMENT):
					information = extract_multiple_positions_arm(rxData);
					if (isCalibrated){
						MCI_State_t motorState =  MC_GetSTMStateMotor1();
						if (motorState == IDLE){
						    // Now restart safely without snap-back, internal reference accurately zeroed
...
bool CalibrateSingleMotor(void){
	...
	/*
	 * This funtion is used to calibrate an arm joint. This function will bring the joint backwards at a certain speed, which is determined
	 * by the constant CALIBRATION_DEGREE_REDUCTION_SPEED. This will occur until the lower limit switch is hit, where it will then slowly move
	 * off of the limit switch. Once off the switch, it sets its current position as 0, and then moves forwards to STARTING_POS which
	 * is an experimentally determined position desied after calibration.
	 *
	 */

	float positionAtCalibration = getCurrentPosition();
    ...
    // Restart motor safely to inch away from limit
    if (!MC_StartMotor1()) {
        uart_debug_print("Start Failed...\r\n");
        return false;
    }
    ...
    //Offset because encoder position is relative to startup
    afterCalibrationOffset = getCurrentPosition();
...
void ControlSingleMotorPositionIncrement(float newPosition){
...
void ControlSingleMotorPosition(float newPosition){

	/*
	 * This function is usde to control a single motor with a position. That is, given an input newPosition in degrees, this function
	 * will procede with moving the motor to the position by internally creating a ramp tp the position only if the esc is currently in the
	 * RUN state. Otherwise it will drop the command.
	 *
	 */
...
void ControlSingleMotorPositionFollower(float newPosition){
	...
	case(RUN):
		...
		uart_debug_print("Motor State: Run\r\n");
		////////////////////////////////////////////////////////////////////////////////////////////////////////
		runSingleMotorPositionFollower(newPosition);
		break;
...
void runSingleMotorPosition(float posSetpointDegrees){
	/*
	 * This function is used to control the motor given a target position. This function expects the inserted position to be in degrees, and will travel
	 * to the given position relative to wherever the motor was calibrated  (ie, wherever start motor was called last). It also performs
	 * some checks on the inputted target to ensure that it is possible to reach that target without violating any of the arm mechanical
	 * restrictions.
	 */
	if (!isValidPos(posSetpointDegrees)){
		////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	float currentPositionDegrees = getCurrentPosition();
	float inputShaftDegreeSetpoint = OutputShaftDegreetoInput(posSetpointDegrees + afterCalibrationOffset);
	float inputShaftPositionInRad = degreesToRad(inputShaftDegreeSetpoint);
	// After startup

	MC_ProgramPositionCommandMotor1(inputShaftPositionInRad, computeTrajectoryTime(currentPositionDegrees,posSetpointDegrees));

// use the following to check if it works fpr stopping mid ramp
...
void runSingleMotorPositionIncrement(float degreeIncrement){
	/*
	 * This function is used in order to make the motor move in increments, buy the angle in degrees sepcified. This function can be called if both the motor is or is not calibrated, as
	 * it can be used in itself to calibrate the motors.
	 */
	...
	//First Check if the position + the change desired will even fit
	if (!isValidPos((float)(currentPositionDegrees + degreeIncrement))) {
		////////////////////////////////////////////////////////////////////////////////////////////////////////
		uart_debug_print("Position setpoint is not valid\r\n");
		////////////////////////////////////////////////////////////////////////////////////////////////////////
		return;
	...
	float inputShaftDegreeSetpoint = OutputShaftDegreetoInput(targetPositionDegrees + afterCalibrationOffset);
	float inputShaftPositionInRad = degreesToRad(inputShaftDegreeSetpoint);
...
void runSingleMotorPositionFollower(float posSetpointDegrees){
	/*
	 * This function is used when softawre is responsible for detailing exactly how the motor should move. In this case, commands are sent at regular
	 * intervals and the arm motor will simply follow the different setpoints, and will not set an internal trajectory. This can be
	 * sangerous if not done properly as there is essentially 0 seconds allocated for the arm to execute a certain motion, thus
	 * if large jumps in setpoitns are inserted the arm will move very speratically.
	 */
	if (!isValidPos(posSetpointDegrees)){
		////////////////////////////////////////////////////////////////////////////////////////////////////////
		uart_debug_print("Position setpoint is not valid\r\n");
	...
	MC_ProgramPositionCommandMotor1(inputShaftPositionInRad,0);

// use the following to check if it works fpr stopping mid ramp
//	HAL_Delay(2000);
//	MCI_StopRamp(&Mci[0]);  // Immediately stop any ongoing motion command

}

void lowerLimitSwitchHit(){
	/*
	 * This function describes the behavior of the joint when the limit switch that is used to calibrate is hit after the calibration sequenced has already passed.
	 * The main logic here is to treat the hit as a calibration, and to recenter the arm afterwards in a position ready for movement
	 */
...
void upperLimitSwitchHit(){

	/*
	 * This is not yet inserted in the logic, but it would be called should a limit switch be triggered in an interruprt
	 */

//	float positioAtLimitHit = getCurrentPosition();
//	  // Move incrementally away from limit switch
//	    while (switch1_opened){
//	    	ControlSingleMotorPositionFollower(positioAtLimitHit - 0.2);
//	    	positioAtLimitHit = getCurrentPosition();
//	    	HAL_Delay(5);
//	    }
    MC_StopMotor1();
    waitForStop();
...
float computeTrajectoryTime(float targetPositionSetpoint, float currentPosition){
	/*
	 * This function computes the time that the motor is allowed to perform a maneouver, such that it always has constant velocity. Both the parameters
	 * its expecting is in degrees
	 */

    float angular_velocity = 1.0f; // Default (shouldn't ever be used)
    switch (ESC_ID) {
        case WAIST:
        	if (isCalibrating){
        		angular_velocity = WAIST_CALIBRATION_ANGULAR_VELOCITY;
        	}
        	else{
        ...
        case SHOULDER:
        	if (isCalibrating){
        		angular_velocity = SHOULDER_CALIBRATION_ANGULAR_VELOCITY;
        	}
        	else{
        ...
        case ELBOW:
        	if (isCalibrating){
        		angular_velocity = ELBOW_CALIBRATION_ANGULAR_VELOCITY;
        	}
        	else{
        		angular_velocity = ELBOW_ANGULAR_VELOCITY;
        	}

            break;
        default:
            // If for some reason ESC_ID is not a valid joint, return 1s as a fallback
            return 1.0f;
    }

    float delta_degrees = fabsf(targetPositionSetpoint - currentPosition);
    // Prevent division by zero (shouldn't ever happen)
    if (angular_velocity < 0.0001f)
        return 1.0f;
...
void waitForRun(){
	/*
	 * A helper funtion that blocks the esc until the RUN state is reached. This is useful when cokmands are sent immediately after a motor is started, since the esc by default will simply
	 *  drop such a command. However, if you wait for it to be in run first, it will then go alwyas react to the command that was sent to it.
	 */
	uint32_t tStart = HAL_GetTick();
...
float inputShaftDegreetoOutput(float mechDegreeInputShaft){
	switch(ESC_ID){
...
float OutputShaftDegreetoInput(float mechDegreeInputShaft){
	switch(ESC_ID){
...
bool isValidPos(float rawPosDegrees){
	...
	//This is debug, so might wanna remove after
	if (isCalibrating){
		return true;
	}
...
float getCurrentPosition() {
	float positionRad = MCI_GetCurrentPosition(&Mci[0]);
	float positionAdjusted = inputShaftDegreetoOutput(radToDegrees(positionRad));
	return (positionAdjusted - afterCalibrationOffset);

}


// Converts IEEE-754 16-bit float to 32-bit float (approximate)
...
float extract_multiple_positions_arm(const uint8_t *rxData) {
    // ESC_ID is assumed to be 8, 9, or 10 → map to index 0–2
    int index = ESC_ID - 8;
    if (index < 0 || index > 2) {
        return 0.0f;
    }

    // Each half float = 2 bytes → index * 2 offset
    uint16_t raw_half;
    raw_half = (uint16_t)(rxData[index * 2 + 1] << 8) | rxData[index * 2];

    float result = half_to_float(raw_half);
    return result;
}
...
// DRIVE IDENTICAL INTERFACE BELOW:
...
extern ADC_HandleTypeDef hadc1;  // Ensure hadc1 is defined in main or elsewhere
...
float Read_Temperature_Celsius(void) {
    ...
    // Convert voltage to temperature using example values
    // NOTE: You must replace this with your actual thermistor or sensor conversion
    float temperature = (voltage - 0.5f) * 100.0f;  // for TMP36-style sensors

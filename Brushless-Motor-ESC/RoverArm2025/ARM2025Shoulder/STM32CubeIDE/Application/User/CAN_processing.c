...
#define SHOULDER_UPPER_LIMIT 110
#define ELBOW_LOWER_LIMIT 0
#define ELBOW_UPPER_LIMIT 100
...
const float SHOULDER_ANGULAR_VELOCITY = 25.0; // Value is in degrees per second
const float ELBOW_ANGULAR_VELOCITY = 60.0; // Value is in degrees per second
...
//        runSingleMotorPositionIncrement(-0.2);
//        float currentPos = getCurrentPosition();
//        float delayTime = computeTrajectoryTime(currentPos - 0.2f, currentPos);
//        HAL_Delay(delayTime + 50);
//    }
//
//    MC_StopMotor1();
//    waitForStop();
//
//
////    MCI_SetCtrlPositionAngle(&Mci[0], 0.0f);
////    MCI_ExecPositionCommand(&Mci[0], 0.0f, 0.0f);
////    MCI_ExecBufferedCommands(&Mci[0]);
//
...
//    waitForStop();
//
//    // Again explicitly set SDK internal reference to zero clearly
//
////		MCI_StopRamp(&Mci[0]);                        // Just to be sure
////		ENC_SetMecAngle(&ENCODER_M1, 0); // directly set it to zero --> see if now the function returns 0, else use a offset
////		MCI_SetCtrlPositionAngle(&Mci[0], 0.0f);
////		MCI_ExecPositionCommand(&Mci[0], 0.0f, 0.0f);
////		MCI_ExecBufferedCommands(&Mci[0]);
////		MC_ProgramPositionCommandMotor1(0, 0);
//
//
////    // Critical again: Command current position explicitly to clear trajectory buffer completely
////    MCI_ExecBufferedCommands(&Mci[0]);
////    MCI_StopRamp(&Mci[0]);
////    // Now restart safely without snap-back, internal reference accurately zeroed
////    if (!MC_StartMotor1()) {
...
////    }
////
////    uart_debug_print("Calibration complete, position accurately zeroed.\r\n");
//
//
//    //if need to do offset trick:
...
void runSingleMotorPosition(float posSetpointDegrees){
	 ...
	 * to the given position relative to wherever the motor was calibrated  (ie, wherever start motor was called last).
	 */
...
void runSingleMotorPositionFollower(float posSetpointDegrees){
	/*
	 * This function is used when softawre is responsible for detailing exactly how the motor should move. In this case, commands are sent at regular intervals and the arm motor will simply follow
	 * the different setpoints, and will not set an internal trajectory
	 */
	if (!isValidPos(posSetpointDegrees)){
		////////////////////////////////////////////////////////////////////////////////////////////////////////
		uart_debug_print("Position setpoint is not valid\r\n");

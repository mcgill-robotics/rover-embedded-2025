...
#define LB_CAN_ID 2
#define RB_CAN_ID 3
#define WAIST_CAN_ID 8
#define SHOULDER_CAN_ID 9
#define ELBOW_CAN_ID 10
...
int ESC_ID = SHOULDER_CAN_ID; // CHANGE THIS ID FOR EACH ESC THAT IS CONNECTED TO THE DRIVE SYSTEM!
volatile bool received_CAN_command = false;
FDCAN_RxHeaderTypeDef rxHeader;
uint8_t rxData[8];
FDCAN_HandleTypeDef hfdcan1;
volatile bool switch1_opened;
volatile bool switch2_opened;

extern volatile bool isCalibrating;
extern volatile bool isCalibrated;
extern void lowerLimitSwitchHit();
extern void upperLimitSwitchHit();
...
//			  // pin low → switch closed -> turn on LED
//				  HAL_GPIO_WritePin(GPIOC, GPIO_PIN_6, GPIO_PIN_SET);
//				  switch1_opened = true;
//
//			} else {
//			  // pin high → switch open → turn LED OFF
...
//
////	else{
////		if (GPIO_Pin == GPIO_PIN_3) // Pin 3 is lower limit switch
////		{
////			if (HAL_GPIO_ReadPin(GPIOB, GPIO_PIN_3) == GPIO_PIN_SET) {
////			  // pin low → switch closed -> turn on LED
////			  HAL_GPIO_WritePin(GPIOC, GPIO_PIN_6, GPIO_PIN_SET);
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
    if (!isCalibrated || isCalibrating)
    {
        ...
        {
            case GPIO_PIN_3:
                switch1_opened = (HAL_GPIO_ReadPin(GPIOB, GPIO_PIN_3) == GPIO_PIN_SET);
                HAL_GPIO_WritePin(GPIOC, GPIO_PIN_6, switch1_opened ? GPIO_PIN_SET : GPIO_PIN_RESET);
...
//                MC_StopMotor1();
            ...
            case GPIO_PIN_4:
                switch2_opened = (HAL_GPIO_ReadPin(GPIOB, GPIO_PIN_4) == GPIO_PIN_SET);
                HAL_GPIO_WritePin(GPIOC, GPIO_PIN_6, switch2_opened ? GPIO_PIN_SET : GPIO_PIN_RESET);
...
int main(void)
{
  ...
  while (1)
  {
	  ...
	  // Parse Without Buffer
//		  if (limitSwitch1Triggered)
//		    {
//		        limitSwitch1Triggered = false;
//		        if (switch1_opened) {
//
//		        	if (isCalibrated && !isCalibrating){
//		        		lowerLimitSwitchHit();
...
//		    }
//
//		    if (limitSwitch2Triggered)
//		    {
//		        limitSwitch2Triggered = false;
//
//		        if (switch2_opened) {
//		        	if (isCalibrated && !isCalibrating){
//		        		upperLimitSwitchHit();
